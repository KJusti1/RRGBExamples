...
ri00000000- developers basement room initialize
...
ri00000000 = function(){
	...
	var Map = function(mapBuild){	
	
		var scopeNew = this;
		scopeNew.mapBuild = mapBuild;
		scopeNew.tileArray = scopeNew.mapBuild.tileArray;
		scopeNew.occupants = scopeNew.mapBuild.occupants;
		
		this.getTileByID = function(id){
			if(id>=0 && id <= scopeNew.mapBuild.x*scopeNew.mapBuild.z-1){
				return(scopeNew.tileArray[Math.floor(id/scopeNew.mapBuild.x)][Math.round(id%scopeNew.mapBuild.x)]);
			}
			else{
				return(null);
			}
		...
		this.getNorthOf = function(tile){
			...
			if((Math.floor(tile.id/scopeNew.mapBuild.x)-1) >= 0){
				return(scopeNew.tileArray[Math.floor(tile.id/scopeNew.mapBuild.x)-1][Math.round(tile.id%scopeNew.mapBuild.x)]);
			}
			else{
				return(null);
			};
		...
		this.getSouthOf = function(tile){
			...
			if((Math.floor(tile.id/scopeNew.mapBuild.x)+1) <= scopeNew.mapBuild.z-1){
				return(scopeNew.tileArray[Math.floor(tile.id/scopeNew.mapBuild.x)+1][Math.round(tile.id%scopeNew.mapBuild.x)]);
			}
			else{
				return(null);
		...
		this.getWestOf = function(tile){
			...
			if(Math.abs(Math.round((tile.id-1))%scopeNew.mapBuild.x) < Math.round((tile.id)%scopeNew.mapBuild.x)){
				return(scopeNew.getTileByID(tile.id-1));
			}
			else{
				return(null);
			};
		...
		this.getEastOf = function(tile){
			...
			if(Math.round((tile.id+1)%scopeNew.mapBuild.x) > Math.round((tile.id)%scopeNew.mapBuild.x)){
				return(scopeNew.getTileByID(tile.id+1));
			}
			else{
				return(null);
		...
		this.getNorthWestOf = function(tile){
			...
			if((Math.round((tile.id-1)%scopeNew.mapBuild.x) < Math.round((tile.id)%scopeNew.mapBuild.x)) && (Math.floor(tile.id/scopeNew.mapBuild.x)-1 >= 0) && ((scopeNew.getNorthOf(tile) != null) && (scopeNew.getWestOf(tile) != null))){
				return(scopeNew.tileArray[Math.floor(tile.id/scopeNew.mapBuild.x)-1][Math.round(tile.id%scopeNew.mapBuild.x)-1]);
			}
			else{
				return(null);
			};
		...
		this.getNorthEastOf = function(tile){
			...
			if(((Math.floor(tile.id/scopeNew.mapBuild.x)-1) >= 0) && (Math.round((tile.id+1)%scopeNew.mapBuild.x) > Math.round((tile.id)%scopeNew.mapBuild.x)) && ((scopeNew.getNorthOf(tile) != null) && (scopeNew.getEastOf(tile) != null))){
				return(scopeNew.tileArray[Math.floor(tile.id/scopeNew.mapBuild.x)-1][Math.round(tile.id%scopeNew.mapBuild.x)+1]);
			}
			else{
				return(null);
		...
		this.getSouthWestOf = function(tile){
			...
			if((Math.abs(Math.round((tile.id-1))%scopeNew.mapBuild.x) < Math.round((tile.id)%scopeNew.mapBuild.x)) && ((Math.floor(tile.id/scopeNew.mapBuild.x)+1) <= scopeNew.mapBuild.z-1) && ((scopeNew.getSouthOf(tile) != null) && (scopeNew.getWestOf(tile) != null))){
				return(scopeNew.tileArray[Math.floor(tile.id/scopeNew.mapBuild.x)+1][Math.round(tile.id%scopeNew.mapBuild.x)-1]);
			}
			else{
				return(null);
		...
		this.getSouthEastOf = function(tile){
			...
			if((Math.round((tile.id+1)%scopeNew.mapBuild.x) > Math.round((tile.id)%scopeNew.mapBuild.x)) && ((Math.floor(tile.id/scopeNew.mapBuild.x)+1) <= scopeNew.mapBuild.z-1) && ((scopeNew.getSouthOf(tile) != null) && (scopeNew.getEastOf(tile) != null))){
				return(scopeNew.tileArray[Math.floor(tile.id/scopeNew.mapBuild.x)+1][Math.round(tile.id%scopeNew.mapBuild.x)+1]);
			}
			else{
				return(null);
		...
		this.moveOccupant = function(username, destinationTileID){
			var i = scopeNew.occupants[r00000000.players[username].tileID].indexOf(username);
			scopeNew.occupants[r00000000.players[username].tileID].splice(i, 1);
			scopeNew.occupants[destinationTileID].push(username);
			scope.players[username].tileID = destinationTileID;
			scope.eventEmitter.emit('tileArrival'+destinationTileID.toString(), {username:username, tileID: destinationTileID});
		};

	};
	Map.prototype.constructor = Map;

	var createMapSeed = function(){
	...
	var Player = function(socket, username, tileID, cpc){
		var scopeNew = this;
		...
		this.backpack = new function(){
			...
			this.howMany = function(backpackObjectID){
				var count = 0;
				for(var i = 0; i<scopeNew.contents.length; i++){
					if(scopeNew.contents[i] == backpackObjectID){
						count = count + 1;
					};
					continue;
				};
			...
			this.removeObject = function(backpackObjectID){
				var i = scopeNew.contents.indexOf(backpackObjectID);
				if(i != -1){
					scopeNew.contents.splice(i,1);
				};
			};

			this.addObject = function(backpackObjectID){
				scopeNew.contents.push(backpackObjectID);
			};
		};
	};
	Player.prototype.constructor = Player;

	var Chest = function(name, triggerTiles, escapeTiles, socketRoomID){
		var scopeNew = this;
		...
		this.open = function(username){
			if(scopeNew.playersInRange.indexOf(username) != -1 && !scopeNew.isOpen){
				console.log(username+" Opened The Chest!");
				scopeNew.isOpen = true;
		...
		this.addContents = function(username, contents){
			...
			for(var i = 0; i<contents.length; i++){
				...
				if(numOfObject <= scope.players[username].backpack.howMany(currentObject)){
					continue;
				}
				else{
					hasContents = false;
			...
			if(scopeNew.playersInRange.indexOf(username) != -1 && hasContents && scopeNew.isOpen){
				for(var i = 0; i<contents.length; i++){
					scope.players[username].backpack.removeObject(contents[i]);
				};
				scopeNew.contents = scopeNew.contents.concat(contents);
			};
		};
		...
		this.removeContents = function(username, contents){
			...
			if(scopeNew.playersInRange.indexOf(username) != -1 && hasContents && scopeNew.isOpen){
				for(var i = 0; i<contents.length; i++){
					scope.players[username].backpack.addObject(contents[i]);
				};
				for(var i = 0; i<contents.length; i++){
					scopeNew.splice(scopeNew.contents.indexOf(contents[i]), 1);
				};
			};
		};
		...
		var attemptAddPlayer = function(username){
			if(scopeNew.playersInRange.indexOf(username) == -1){
				scope.players[username].socket.join(socketRoomID);
				scopeNew.playersInRange.push(username);
				//emit chest interaction with contents to joining player
			};
		};

		var attemptRemovePlayer = function(username){
			if(scopeNew.playersInRange.indexOf(username) != -1){
				scope.players[username].socket.leave(socketRoomID);
				var i = scopeNew.playersInRange.indexOf(username);
				scopeNew.playersInRange.splice(i, 1);
				//emit remove chest interaction from leaving player
			};
		};

		for(var i = 0; i<scopeNew.triggerTiles.length; i++){
			scope.eventEmitter.on('tileArrival'+scopeNew.triggerTiles[i].toString(), function(e){attemptAddPlayer(e.username);});
		};

		for(var i = 0; i<scopeNew.escapeTiles.length; i++){
			scope.eventEmitter.on('tileArrival'+scopeNew.escapeTiles[i].toString(), function(e){attemptRemovePlayer(e.username);});
		};
	};
	...
	//map
	scope.map = createMapSeed();

	//interactives
	scope.chests = {};

	//make chests
	scope.chests['c00000000'] = new Chest('c00000000', [15], [30], '00000004');

  	var movementVelocity = 3;

	scope.enter = function(si){
		var newCPC = createCPC(si.username);
		...
		var newPlayer = new Player(si.socket, si.username, newPlayerTileID, newCPC);
